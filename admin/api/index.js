...
const orderByCategory = (sid, tid, asc = true) => {
  if (sid && tid) {
    let url = `${util.parseUri(urls.ORDER_CATEGORY, false)}/${sid}/${
      asc ? "back" : "front"
    }/${tid}`;
    return post(url);
  } else {
...
const fileUpload = (params, onUploadProgress, cancelSource) => {
  ...
  return post(url, {
    ...
    onUploadProgress: progressEvent =>
      onUploadProgress &&
      onUploadProgress.call(undefined, progressEvent, cancelSource)
  });
};
...
const fetchTagList = (params = {}) => {
  ...
  let url = `${util.parseUri(
    urls.FETCH_TAG_LIST,
    false
  )}/${page}/${size}/${type}/${orders}`;
  return get(url);
};
